...
class DetectionErrorBloc implements Bloc {
  var stateDetectionError = DetectionErrorState(
  ...
  DetectionErrorBloc() {
    ...
    eventController.stream.listen((event) async {
      if (event is FetchDetectionError) {
        ...
        List rejectedUndefinedErrors = await UndefinedErrorDAO()
            .fetchUndefinedErrorRejected(event.isRejected);
        List rejectedDetectionsErrors = await DetectionErrorDAO()
            .fetchDetectionErrorRejected(event.isRejected);
        List<DetectionError> listDetectionError = detectionErrors;
        List<UndefinedError> listUndefinedError = undefinedErrors;
        List<DetectionError> listDetectionErrorRejected =
            rejectedDetectionsErrors;
        List<UndefinedError> listUndefinedErrorRejected =
            rejectedUndefinedErrors;
        var newList = new List.from(listDetectionErrorRejected)
          ..addAll(listUndefinedErrorRejected);
